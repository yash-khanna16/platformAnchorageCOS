...
const Cart: React.FC<CartPropsType> = ({ cartOpen, setCartOpen, expandedId, toggleExpand }) => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (auth) {
      try {
        ...
        const items = cartEssentials.map((item: CartType) => {
  ...
  useEffect(() => {
    ...
    let totalAmount = 0;
    cartEssentials.forEach((element) => {
      totalAmount += element.price * element.quantity;
    });
    if (cartEssentials.length === 0) setCartOpen(false);
  ...
  return (
    <div className="font-montserrat py-2">
      <div style={{ height: "100vh" }}>
        ...
        {cartEssentials.map((item: CartType) => (
          ...
          <DialogContent className="h-fit">
            ...
            {notAvailable.map((item_not_available, index: number) => (
              <div key={index} className="flex justify-between border-b border-dashed">
                ...
                <div className="mr-2">{cartEssentials.find((item)=> item.item_id === item_not_available.item_id)?.quantity}</div>
              </div>
            ))}
